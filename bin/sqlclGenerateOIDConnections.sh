...
function sqlclGenerateOIDConnections() {
    ############################################################################
    #
    # Notes
    #
    ...
    ############################################################################
    function usage() {
        printf -- 'This script is used to generate SQLcL named connections based on Oracle Internet Directory (OID) LDAP server entries\n'
        printf -- '\n'
        printf -- 'The following arguments are recognized\n'
        printf -- '\n'
        printf -- '  -b {binary}    --Specify the SQLcl binary to use\n'
        printf -- '  -B {base}      --The LDAP base from where the script should start searching for database entries\n'
        ...
        printf -- '  -i {context}   --Context from LDAP to include for connection generation\n'
        printf -- '                 --Can be specified more than once\n'
        ...
        printf -- '\n'
        printf -- 'Example:\n'
        printf -- '  %s -H "example.ldap-host.com" -u "db_user1" -u "db_user2"\n' "${scriptName}"
        printf -- '\n'
    ...
    function parseLdapSearchResponse() {
        ...
        #   attributes: blank to return all attributes, or comma separated list of attributes to return
        #   scope:
        ...
        #   Separate each attribute (that is not first attribute or last attribute) onto its own line by replacing each group seperator (GS) by a newline
        ...
        #   Separate attribute name and attribute value onto their own lines
    ...
    function parseContextDatabases() {
        local ldapHost="${1}"
        local ldapPort="${2}"
        local ldapDatabaseSearchUrl="${3}"
        local ldapconBase="${4}"
        local recordSeparator="${5}"
        local rootFolder="${6}"
        local context="${7}"
        local connectionsFile="${8}"
        local sqlFile="${9}"
        local -a userList=("${@:10}")
        ...
        while read -r line; do
            if [[ "${line}" == "${recordSeparator}" ]]; then
                ...
                for user in "${userList[@]}"; do
                    ...
                    connectionName="${user}@${databaseName}"

                    {
                        printf -- '        ,{\n'
                        printf -- '                "info": {\n'
                        printf -- '                "OracleConnectionType": "ADVANCED",\n'
                        printf -- '                "oraDriverType": "thin",\n'
                        printf -- '                "subtype": "oraJDBC",\n'
                        printf -- '                "customUrl": "jdbc:oracle:thin:@ldap://%s:%s/%s,cn=%s,%s",\n' "${ldapHost}" "${ldapPort}" "${databaseName}" "${context}" "${ldapconBase}"
                        printf -- '                "user": "%s"\n' "${user}"
                        printf -- '            },\n'
                        printf -- '            "name": "%s",\n' "${connectionName}"
                        printf -- '            "type": "jdbc"\n'
    ...
    local -a connectionsFiles=()
    local sqlFile
    local -a sqlFiles=()
    local pid
    local -a pidList=()
    local contextConnectionFile
    local contextSqlFile
    local sqlclStdOutput
    local sqlclStdError
    ...
    if [[ "${verboseLevel}" -eq 0 ]]; then
        sqlclStdOutput='/dev/null'
        sqlclStdError='/dev/null'
    else
        sqlclStdOutput='/dev/fd/1'
        sqlclStdError='/dev/fd/2'
    fi
    ...
    if ! wait "${pidList[@]}"; then
        printf -- 'ERROR: One or more context LDAP/database parsing jobs failed\n' >&2
        return 21
    fi
    ...
    printf -- '\n'
    printf -- '%s\n' "${h1}"
    printf -- '%s %s\n' "${hs}" "Generate SQLcl Connection File"
    printf -- '%s\n' "${h1}"
...

    {
        ...
        for contextConnectionFile in "${connectionsFiles[@]}"; do
            cat "${contextConnectionFile}"
            rm "${contextConnectionFile}"
        done
    ...
    printf -- '\n'
    printf -- '%s\n' "${h1}"
    printf -- '%s %s\n' "${hs}" "Generate SQLcl SQL script for connection import"
    printf -- '%s\n' "${h1}"
    ...
    printf -- '\n'
    printf -- '%s\n' "${h1}"
    printf -- '%s %s\n' "${hs}" "Execute SQLcl SQL Script"
    printf -- '%s\n' "${h1}"

    if [[ "${verboseLevel}" -eq 0 ]]; then
        printf -- '\n'
    fi
    SQLPATH="" "${sqlclBinary}" -S -nolog -noupdates -nohistory @"${sqlFile}" 1>"${sqlclStdOutput}" 2>"${sqlclStdError}"

    rm "${connectionsFile}" "${sqlFile}"
    ...
    printf -- '\n'
    printf -- '%s\n' "${h1}"
    printf -- '%s %s\n' "${hs}" "Generate SQLcl SQL script for folder organization"
    printf -- '%s\n' "${h1}"
    ...
    {
        for contextSqlFile in "${sqlFiles[@]}"; do
            cat "${contextSqlFile}"
            rm "${contextSqlFile}"
        done
    ...
    printf -- '\n'
    printf -- '%s\n' "${h1}"
    printf -- '%s %s\n' "${hs}" "Execute SQLcl SQL script for folder organization"
    printf -- '%s\n' "${h1}"
